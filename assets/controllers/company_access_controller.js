...
export default class extends Controller {
    static targets = ['modal', 'backdrop', 'search', 'list', 'counter', 'summary', 'hiddenInputs', 'newName', 'newSiret', 'newError'];
    static values = {
    ...
    _refreshSummary() {
        ...
        if (checked.length === 0) {
            ...
            const names = checked.map((cb) => cb.dataset.name).slice(0, 5);
            const more = checked.length > 5 ? ` +${checked.length - 5} autres` : '';
            this.summaryTarget.innerHTML = names
                .map((n) => `<span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-[var(--color-primary-light)] text-[var(--color-primary)] text-xs font-medium">${this._escape(n)}</span>`)
                .join(' ') + (more ? `<span class="text-xs text-[var(--color-secondary)] ml-1">${more}</span>` : '');
        }
    }
    ...
    async createNew(event) {
        ...
        try {
            ...
            // Injecte la nouvelle ligne cochée en tête de liste
            ...
            row.innerHTML = `
                <input type="checkbox" value="${data.id}" data-name="${this._escape(data.name)}" data-action="change->company-access#toggle" checked
                ...
                <div class="flex-1 min-w-0">
                    ...
                    <div class="text-xs text-[var(--color-secondary)]">${this._escape(data.status_label)}</div>
                </div>
            `;
